...
function createNewScript(
    ...
    toBeAddedScript.setAttribute(WP_PROXY_ID, wpProxyId);
...
function rewriteSrc(node: HTMLScriptElement, newSrc: string, wpProxyId: string, onload = noop) {
    console.log('rewriteSrc', newSrc);
    // 需要先设置标识位，赋值src和type会检查WP_PROXY_ID
    node.setAttribute(WP_PROXY_ID, wpProxyId);
    node.src = newSrc;
...
export function scriptRewrite(node: HTMLScriptElement, newSrc: string, onload = noop) {
    ...
    if (!isAsyncScript && document.readyState === 'loading') {
        ...
        if (navigator.userAgent.toLowerCase().indexOf('firefox') === -1) {
            ...
            rewriteSrc(node, newSrc, wpProxyId, onload);
        }
        // Firefox or Others
        else {
            // FIXME: 同步script转为异步时，可能丢失DOMContentLoaded事件回调
            // newScript(node, newSrc, wpProxyId, onload);
            node.async = false;
            node.defer = true;
            createNewScript(node, newSrc, wpProxyId, onload);
        }
    }
    // 动态加载的异步script标签
    else {
